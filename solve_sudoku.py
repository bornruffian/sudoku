...
def process(frame):
    ...
    ret,blurred = cv2.threshold(gray,245,255,0)
    
    print 'mean = ' + str(cv2.mean(gray))
        
    #ret,blurred = cv2.threshold(gray,128,255,cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    ...
    #
    #cv2.imshow('not',cv2.bitwise_not(blurred))
    contours, _ = cv2.findContours(image=cv2.bitwise_not(blurred),
                                   mode=cv2.RETR_TREE, #mode=cv2.RETR_LIST,
                                   method=cv2.CHAIN_APPROX_NONE) #method=cv2.CHAIN_APPROX_SIMPLE)
    sudoku_area = 0
    sudoku_contour = None
    ...
    for cnt in contours:
        ...
        #print "%d %d %d %d" % (0.7 < (float(w) / h) < 1.3, area > 150*150, area > sudoku_area, area > .5 * w * h)
    ...
    if sudoku_contour is not None:
        im = frame
        cv2.drawContours(im,sudoku_contour,-1,(255,255,0),3)
        cv2.imshow("final contour",im)        
        ...
        if len(approx) == 4:
            ...
            cv2.drawContours(mask_x, sorted_contours[:11], -1, 255, -1)
            if args.debug:
            ...
            cv2.drawContours(mask_y, sorted_contours[:11], -1, 255, -1)
            if args.debug:
            ...
            #cv2.imshow('dilated_hor',dilated_hor)            
            #cv2.imshow('dilated_ver',dilated_ver)
            
            crossing = cv2.bitwise_and(dilated_hor, dilated_ver)

            #print crossing
            
            mask_t = np.zeros(crossing.shape, np.uint8)
            mask_t[sx:sx+sw,sy:sy+sh] = 1            
            t=cv2.bitwise_and(mask_t,crossing)
            cv2.imshow('grid',grid)
            
            #mini = crossing.shape[0]
            #minj = crossing.shape[1]
            #m = 0
            #for ti in range(crossing.shape[0]):
            #    for tj in range(crossing.shape[1]):
            #        if crossing[ti][tj] > m:
            #           m = crossing[ti][tj]
            #        if crossing[ti][tj] == 255:
            #           if (ti < mini):
            #              mini = ti
            #           if (tj < minj):
            #              minj = tj
            ...
            #cv2.imshow('precrossing',crossing)
            #crossing[0:max(sx-5,0),:] = 0
            #crossing[:,0:max(sy-5,0)] = 0
            #crossing[sx+sw+10:-1,:] = 0
            #crossing[:,sy+sh+10:-1] = 0
            crossing[0:sy+10,:] = 0
            cv2.imshow('postcrossing',crossing)
            ...
            if len(contours) == 100:
                ...
                print "Not enough/correct contours, " + str(len(contours))
        else:
            print "Not 4 lines, " + str(len(approx))
    else:
        print "No sudoku contour"
...
def solve_sudoku_ocr(src, crossing_points):
    ...
    for i, pos in enumerate([pos for pos in range(len(crossing_points)-11) if (pos + 1) % 10 != 0]):
        ...
        #print "Waiting for usr"
        #s = raw_input()
        #tesseract.SetCvImage(ipl, api)
        #ocr_text = api.GetUTF8Text()
        
        cv2.imwrite('tmp.png',transformed)
        ocr_text = pytesseract.pytesseract.image_to_string(Image.open('tmp.png'),config='--psm 6 outputbase digits')        
        print 'Got: ' + str(ocr_text)
    ...
    print 'numbers shape = ' + str(len(numbers))
    for x in range(9):
        for y in range(9):
    ...
    for i in range(9):
        for j in range(9):
            if numbers[n] >= 1 and numbers[n] <= 9:
                grid[i][j] = numbers[n]
            n += 1
